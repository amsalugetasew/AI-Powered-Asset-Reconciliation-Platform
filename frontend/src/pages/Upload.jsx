...
const Upload = () => {
  ...
  return <div className="space-y-5 pb-10">
    ...
    <div className="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm"><div className="flex items-center gap-2 border-b border-gray-50 px-5 py-4"><FiClock className="h-4 w-4 text-gray-400" /><h2 className="text-sm font-bold text-gray-800">Recent Upload History</h2></div><div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-gray-50">{['Job ID', 'Date & Time', 'ERP File', 'Physical Count File', 'Records', 'Status', 'Actions'].map(header => <th key={header} className="px-5 py-3 text-left text-[11px] font-bold uppercase tracking-wider text-gray-400">{header}</th>)}</tr></thead><tbody className="divide-y divide-gray-50">{history.length === 0 ? <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-gray-400">No upload history yet. Upload files above to get started.</td></tr> : history.map(record => <tr key={record.id} className="transition-colors hover:bg-gray-50/60"><td className="px-5 py-3.5 text-xs font-bold text-[#8E288D]">AR-{String(record.id).padStart(4, '0')}</td><td className="whitespace-nowrap px-5 py-3.5 text-xs text-gray-500">{formatDate(record.created_at)}</td><td className="max-w-[180px] truncate px-5 py-3.5 text-xs text-gray-700" title={shortName(record.internal_file)}>{shortName(record.internal_file)}</td><td className="max-w-[180px] truncate px-5 py-3.5 text-xs text-gray-700" title={shortName(record.customer_file)}>{shortName(record.customer_file)}</td><td className="whitespace-nowrap px-5 py-3.5 text-xs font-semibold text-gray-800">{((record.statistics?.total_internal_records || record.total_internal_records || 0) + (record.statistics?.total_customer_records || record.total_customer_records || 0)).toLocaleString()}</td><td className="px-5 py-3.5"><StatusPill status={record.status} /></td><td className="px-5 py-3.5"><button onClick={() => navigate(`/results/${record.id}`)} className="inline-flex items-center gap-1 text-xs font-semibold text-[#8E288D] hover:text-[#7A1E79]"><FiEye className="h-3.5 w-3.5" />View</button></td></tr>)}</tbody></table></div></div>
    {historyPageCount > 1 && (
